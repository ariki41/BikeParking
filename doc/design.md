...
| `prefectures` / `cities` / `postalcodes` | 住所マスタ | 日本郵便のUTF-8版データから月次同期する郵便番号と住所の対応。廃止郵便番号は参照保持のため無効化 |
