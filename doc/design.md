...
利用者向けの文言と本設計書では、二輪車を対象とする施設を「駐輪場」と表記する。DB・URL・クラス名の `ParkingSpot` は技術上の識別子として維持する。
...
| 駐輪場通報 | GET/POST | `/parking-spots/{parkingSpot}/reports/create`, `/parking-spots/{parkingSpot}/reports` | 必須 |
| 通報管理・掲載停止 | GET/POST | `/admin/parking-spot-reports`, `/admin/parking-spots/{parkingSpot}/hide` | 管理者 |
...
erDiagram
    ...
    users o|--o{ parking_spot_images : adds
...
| `parking_spot_images` | 駐輪場画像 | 対象駐輪場、追加者（退会時は `NULL`）、保存パス、表示順 |
...
- 駐輪場画像の追加者を記録し、マイページでは本人が追加した画像だけを新しい順に表示する。既存画像は追加者を推測して誤帰属させないため `user_id` を未設定のまま保持し、投稿画像一覧には表示しない。退会時は画像レコード・ファイルを保持し、追加者は `NULL` に匿名化する。
- 料金帯は 1〜4 件。
- 料金帯の `day_type` は適用曜日（全日・平日・土日祝）だけを表し、時間帯は開始時刻・終了時刻で表す。旧「昼間」「夜間」データは時刻を維持したまま `全日` へ移行するため、元の表示用名称は復元しない。
...
- レビューの評価は 1〜5、コメントは必須かつ1,000文字以内とする。評価範囲はリクエスト検証とDBの `CHECK` 制約で保証し、既存の範囲外値はマイグレーション時に最小値（1）または最大値（5）へ補正する。
- レビューは1ユーザーにつき1駐輪場1件とし、再投稿時は本人の既存レビューを更新する。投稿者本人だけが更新でき、削除機能は提供しない。
- お気に入りは1ユーザーにつき1駐輪場1件とし、追加・解除はログインユーザー自身の関連だけを操作する。重複は既存データを最も古い1件だけ残して整理した後、DBの一意制約で防止する。
- 利用者が退会した場合、登録駐輪場・料金・画像・タグ関連・他者のお気に入りとレビューは継続表示のため保持し、所有者・画像追加者・レビュー投稿者・更新者は `NULL` に匿名化する。退会者自身のお気に入りだけを削除する。
- 同一駐輪場の同一タグ関連と、同一曜日区分・開始時刻・終了時刻の料金帯はDBの一意制約で重複を防止する。地図範囲検索に使う緯度・経度には複合インデックスを設定する。
- 駐輪場の編集画面では、確認後に閉鎖済みとして掲載停止にできる。掲載停止した駐輪場は検索結果に「閉鎖済み」と表示し、利用者は「閉鎖済みを除外」フィルターで非表示にできる。料金・画像・お気に入り・レビュー・更新履歴および画像ファイルは保持する。誤登録は削除申請として管理画面へ送り、管理者が削除理由・実行者・日時を記録して確認後に物理削除する。物理削除時は料金・画像・お気に入り・レビュー・更新履歴・タグ・通報・運営操作履歴を削除し、駐輪場画像ファイルもDBコミット後に削除する。削除申請自体は対象名と管理者の判断記録を保持する。
