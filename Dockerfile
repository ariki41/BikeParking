...
ENV APP_ENV=production APP_DEBUG=false APACHE_DOCUMENT_ROOT=/var/www/html/public
RUN apt-get update && apt-get install -y --no-install-recommends curl libfreetype6-dev libicu-dev libjpeg62-turbo-dev libpng-dev libwebp-dev libzip-dev unzip \
...
RUN ln -sfn ../storage/app/public public/storage && chmod +x /usr/local/bin/app-entrypoint && chown -R www-data:www-data bootstrap/cache storage
