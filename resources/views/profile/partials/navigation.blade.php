<nav class="border-b border-gray-200 dark:border-gray-700" aria-label="マイページ">
    <div class="flex gap-6 overflow-x-auto">
        ...
        <a class="border-b-2 px-1 pb-3 text-sm font-medium {{ request()->routeIs('profile.parking-spots') ? 'border-emerald-600 text-emerald-700 dark:text-emerald-400' : 'border-transparent text-gray-600 hover:border-gray-300 hover:text-gray-800 dark:text-gray-400 dark:hover:text-gray-200' }}" href="{{ route('profile.parking-spots') }}" @if (request()->routeIs('profile.parking-spots')) aria-current="page" @endif>
            登録した駐輪場
        </a>
        <a class="border-b-2 px-1 pb-3 text-sm font-medium {{ request()->routeIs('profile.edited-parking-spots') ? 'border-emerald-600 text-emerald-700 dark:text-emerald-400' : 'border-transparent text-gray-600 hover:border-gray-300 hover:text-gray-800 dark:text-gray-400 dark:hover:text-gray-200' }}" href="{{ route('profile.edited-parking-spots') }}" @if (request()->routeIs('profile.edited-parking-spots')) aria-current="page" @endif>
            編集した駐輪場
        </a>
    </div>
</nav>
